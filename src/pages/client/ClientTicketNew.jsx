...
export default function ClientTicketNew() {
  ...
  async function handleSubmit(event) {
    ...
    const { data: ticketId, error: createError } = await supabase.rpc(
    ...
    if (createError || !ticketId) {
      console.error("Failed to create ticket:", createError?.message || "No ticket id returned");
      setError("We could not create your ticket right now. Please try again.");
      return;
    }

    navigate(`/client/tickets/${ticketId}`, { replace: true });
  }
      ...
      <div className="card rounded-3xl p-6 md:p-8">
        <form onSubmit={handleSubmit} className="grid gap-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/client/tickets" className="btn btn-secondary">Cancel</Link>
          </div>
        </form>
